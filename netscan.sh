...
if [[ -z "$TARGET" ]]; then
    ...
    echo "  target: single IP (192.168.1.50) or CIDR /24 (192.168.1.0/24)"
    echo "  port-range: e.g. 1-100 (default) or 1-1000"
    exit 1
fi
...
for host in "${hosts[@]}"; do
    ( timeout "$(( (CONNECT_TIMEOUT + BANNER_TIMEOUT) * (PORT_END - PORT_START + 1) + 2))" bash -c "scan_host '$host'" ) &
    ((running++))
